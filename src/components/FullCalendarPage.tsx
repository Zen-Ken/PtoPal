...
import { Calendar, Clock, TrendingUp, DollarSign, Plus, X, Edit3, Trash2, MapPin, Save } from 'lucide-react';
...
export default function FullCalendarPage({ onBack, userSettings, onUpdateSettings }: FullCalendarPageProps) {
  ...
  // Transform vacations into FullCalendar events
  const calendarEvents = useMemo(() => {
    const events: any[] = [];

    // Add vacation events
    userSettings.vacations.forEach((vacation, index) => {
      const colors = [
      ...
      const endDate = new Date(createDateFromString(vacation.endDate));
      ...
      events.push({
        ...
        allDay: true,
        className: `vacation-event bg-gradient-to-r ${colorClass}`,
        extendedProps: {
          type: 'vacation',
          vacation: vacation,
          totalHours: vacation.totalHours,
          includeWeekends: vacation.includeWeekends
        }
      });
    });

    // Add pay day events for current month view
    ...
    if (userSettings.payPeriod === 'semimonthly') {
      ...
      while (currentDate <= endOfMonth) {
        ...
        if (firstPayDate >= startOfMonth && firstPayDate <= endOfMonth) {
          ...
          events.push({
            id: `payday-${firstPayDate.toISOString().split('T')[0]}`,
            title: `Pay Day - ${totalPTO.toFixed(2)} hrs`,
            start: firstPayDate.toISOString().split('T')[0],
            allDay: true,
            className: 'payday-event',
            extendedProps: {
              type: 'payday',
        ...
        if (secondPayDate >= startOfMonth && secondPayDate <= endOfMonth) {
          ...
          events.push({
            id: `payday-${secondPayDate.toISOString().split('T')[0]}`,
            title: `Pay Day - ${totalPTO.toFixed(2)} hrs`,
            start: secondPayDate.toISOString().split('T')[0],
            allDay: true,
            className: 'payday-event',
            extendedProps: {
              type: 'payday',
      ...
      while (currentPayDate <= endOfMonth) {
        if (currentPayDate >= startOfMonth) {
          ...
          events.push({
            id: `payday-${currentPayDate.toISOString().split('T')[0]}`,
            title: `Pay Day - ${totalPTO.toFixed(2)} hrs`,
            start: currentPayDate.toISOString().split('T')[0],
            allDay: true,
            className: 'payday-event',
            extendedProps: {
              type: 'payday',
  ...
  const handleDateClick = (dateClickInfo: any) => {
    ...
    const vacationsForDay = getVacationsForDate(new Date(clickedDate), userSettings.vacations);

    if (vacationsForDay.length > 0) {
      // Edit existing vacation
      const vacation = vacationsForDay[0];
  ...
  const handleEventClick = (eventClickInfo: any) => {
    ...
    if (event.extendedProps.type === 'vacation') {
      const vacation = event.extendedProps.vacation;
      setEditingVacation(vacation);
      setVacationForm({
        startDate: vacation.startDate,
        endDate: vacation.endDate,
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Full Calendar View</h1>
                <p className="text-gray-600 dark:text-gray-400">Advanced calendar with FullCalendar integration</p>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/30 dark:to-orange-900/30 rounded-xl p-6 border border-amber-200/50 dark:border-amber-700/50">
              ...
              <div className="space-y-3">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-sm text-gray-700 dark:text-gray-300">Pay Day Events</span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="w-4 h-4 bg-gradient-to-r from-purple-500 to-pink-600 rounded"></div>
                  <span className="text-sm text-gray-700 dark:text-gray-300">Vacation Events</span>
                </div>
                <div className="text-xs text-gray-600 dark:text-gray-400 mt-3 p-2 bg-gray-100 dark:bg-gray-700 rounded">
                  <strong>Tip:</strong> Click on events to edit, or click on any day to add new vacations
                </div>
              </div>
            </div>
          </div>
